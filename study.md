...
## 안드로이드 스레드  
### Main 스레드
Main 스레드는 UI를 변경하는 모든 코드를 실행하며, 액티비티 시작, 버튼누름 등 서로 다른 UI 관련 이벤트들에 대한 응답으로 실행되는 코드들도 포함,  
어떤 형태로든 모두 UI와 관련 있어서 main스레드를  UI스레드라고도 한다.  
  
애플리케이션이 응답하지 않음(Application Not Responding, ANR)  
main스레드가 중요한 이벤트에 대한 응답에 실패했다고 안드로이드의 와치독이 판단하면 ANR이 발생, 애플리케이션 중단  
(10초 이내에 UI 응답이 없으면 사용자가 불편하지 않도록 안드로이드 시스템에서 ANR 발생)  

### 백그라운드 스레드  
장시간 실행되는 모든 작업은 백그라운드 스레드로 수행되어야 한다  
main 스레드가 UI 관련 작업을 자유롭게 처리하기 위해, 사용자를 위한 UI 응답을 지속적으로 처리할 수 있음  

UI는 메인 스레드에서만 변경할 수 있다.  
백그라운드 스레드에서 UI를 변경하려고 하면 에러가 발생  
...
## Executors  
스레드를 참조하는 객체, execute 라는 함수를 가지며 이 함수는 실행할 코드 블록을 받는다.  
Executors 인스턴스를 생성하면 이 인스턴스가 새로운 백그라운드 스레드를 사용해 블록의 코드를 실행한다.  
즉 amin 스레드를 방해하지 않고 DB 작업을 안전하게 수행할 수 있다.  
(DB 작업 시 사용하려면 DAO 인터페이스 에서 하면 안됨)  
(또한 Room이 변경(update), 추가(insert)의 경우에는 Room이 백그라운드 스레드로 자동 실행하지 못하기 때문에 백그라운드 스레드로 변경이나 추가 함수들을 호출 해야하는데 이때 주로 executor를 사용함)  
...
데이터베이스가 모델 데이터를 처리할 수 있게 타입 변화기(type converter)를 생성한다  
@Database 어노테이션을 사용하면 사용한 클래스가 이 앱의 데이터베이스임을 Room에게 알려줌  
정의한 엔터티를 기반으로 앱의 데이터베이스 테이블 구조를 만듦, 엔터티는 우리가 생성하는 모델 클래스 (@Entity 사용)

## 리포지터리 패턴  
데이터베이스 액세스를 위해 리포지터리 패턴을 사용(구글 앱 아키텍처 지침에서 권장)  
리포지터리(데이터 저장소)를 구현함
